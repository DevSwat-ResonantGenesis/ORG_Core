...
class SandboxConfig:
    ...
    # Network restrictions (empty allowed_hosts = allow all public hosts; denied_hosts still blocks)
    allowed_hosts: Set[str] = field(default_factory=set)
    denied_hosts: Set[str] = field(default_factory=lambda: {"169.254.169.254", "metadata.google.internal"})  # Cloud metadata
...
class SandboxBoundary:
    ...
    def reset_session(self) -> None:
        """Reset session counters — call before each new agent execution."""
